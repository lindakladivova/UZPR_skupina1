...
$output_dir = 'D:\CVUT\155UZPR\GitHub\UZPR_skupina1\data\geojson_obec_boskovice_4326\'
...
foreach ($ku in $seznam_ku) {
    ...
    foreach ($shapefile in $shapefiles) {
        ...
        ogr2ogr -f "GeoJSON" -a_srs $PSScriptRoot\sour_system_krovak.prj  -t_srs EPSG:4326 $output_dir\$ku_name`_$shp_name`.geojson $ku_fullname\$shp_name`.shp
